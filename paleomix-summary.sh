...
for sample in $SAMPLES; do
	...
	hits_raw=$(awk -v sample=${sample} '$3 == sample && $4 == "hits_raw(${ref})" {print $5}' ${sumdir}${sample}.summary)
	hits_raw_frac=$(awk -v sample=${sample} '$3 == sample && $4 == "hits_raw_frac(${ref})" {print $5}' ${sumdir}${sample}.summary)
	hits_clonality=$(awk -v sample=${sample} '$3 == sample && $4 == "hits_clonality(${ref})" {print $5}' ${sumdir}${sample}.summary)
	hits_unique=$(awk -v sample=${sample} '$3 == sample && $4 == "hits_unique(${ref})" {print $5}' ${sumdir}${sample}.summary)
	hits_unique_frac=$(awk -v sample=${sample} '$3 == sample && $4 == "hits_unique_frac(${ref})" {print $5}' ${sumdir}${sample}.summary)
	hits_coverage=$(awk -v sample=${sample} '$3 == sample && $4 == "hits_coverage(${ref})" {print $5}' ${sumdir}${sample}.summary)
	hits_length=$(awk -v sample=${sample} '$3 == sample && $4 == "hits_length(${ref})" {print $5}' ${sumdir}${sample}.summary)
